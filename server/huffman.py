...
class HuffmanCoding:
	...
	def make_codes_helper(self, root, current_code):
		...
		if(root.char != None):
			...
			#print(self.reverse_mapping)
			file5,file5_ext=os.path.splitext(self.path)
			with open(file5 + '.json', 'w') as f:
				json.dump(self.reverse_mapping, f)			
			return
...
class HuffmanCoding1:
	def __init__(self,path):
		self.path=path
		file6,file6_ext=os.path.splitext(self.path)
		with open(file6+'.json') as f:
	...
	def decode_text(self, encoded_text):
		current_code = ""
		decoded_text = ""
		#self.reverse_mapping=HuffmanCoding(self,path)
